...
public class Dave_Visa_ implements PlugIn 
{
	...
	public void run(String arg) 
	{
		...
		ImagePlus impseg;
		ImagePlus impJ;
		IJ.showMessage("Algorithme Dave","If ready, Press OK");
		ImagePlus cw;
		...
		impseg=NewImage.createImage("Image segmentée par Dave",width,height,1,24,0);
		...
		for(k=0; k<nbpixels; k++) {
			...
			for (i=0; i<nbclasses; i++) {
				...
				for(j=0; j<kmax; j++){
					if (Dprev[j][k] != 0.0)
						Umat[i][k]+=Math.pow((Dprev[i][k]/Dprev[j][k]),(2/(m-1)));
					else
						Umat[i][k]+=1.0;
				}
				Umat[i][k]=((int)Umat[i][k]);
				Umat[i][k]=1/Umat[i][k];
				sum+=Umat[i][k];
			}
			Umat[i][nbclasses]=1-sum;
		}
		...
		while ((iter < itermax) && (stab > seuil)) 
		{
			...
			delta /= nbclasses*nbpixels;
			delta *= lambda;
			for (j = 0; j < nbpixels; j++) {
				for (i = 0; i < nbclasses; i++) {
					performanceIndex += Math.pow(Umat[i][j],m) * Dmat[i][j];
				}
				performanceIndex += delta * Math.pow(Umat[nbclasses][j],m);
			}
