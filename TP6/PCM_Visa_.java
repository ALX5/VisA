...
public class PCM_Visa_ implements PlugIn 
{
		...
		while ((iter < itermax) && (stab > seuil)) 
		{
			...
			for(i=0; i<nbclasses; i++) {
				...
				for(j=0; j<nbpixels; j++) {
					n[i]+=Math.pow(Umat[i][j], m)*Dprev[i][j];
					sum+=Math.pow(Umat[i][j], m);
				}
				n[i]=n[i]/sum;
			...
			for(i=0; i<nbclasses; i++) {
				n[i]=0;
				for(j=0; j<nbpixels; j++) {
					n[i]+=Math.pow(Umat[i][j], m)*Dmat[i][j];
					sum+=Math.pow(Umat[i][j], m);
				}
				if(sum!=0) n[i]=n[i]/sum;
			}			
